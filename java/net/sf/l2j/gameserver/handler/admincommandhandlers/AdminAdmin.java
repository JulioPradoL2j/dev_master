...
public class AdminAdmin implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.startsWith("admin_kill"))
		{
			...
			Player player = L2World.getInstance().getPlayer(firstParam);
			if (player != null)
			{
				if (st.hasMoreTokens())
				{
					...
					if (StringUtil.isDigit(secondParam))
					{
						...
						for (Creature knownChar : player.getKnownList().getKnownTypeInRadius(Creature.class, radius))
						{
			...
			else if (StringUtil.isDigit(firstParam))
			{
				...
				for (Creature knownChar : activeChar.getKnownList().getKnownTypeInRadius(Creature.class, radius))
				{
					if (knownChar.equals(activeChar))
						continue;
		...
		else if (command.startsWith("admin_dailyreward"))
		{
			...
			try
			{
				do
				{
					...
					else if (type.startsWith("dressme"))
					{
						...
						activeChar.sendMessage("The content of DressMe.xml has been reloaded.");
					}
					...
					{
						FusionItemData.getInstance().reload();
						activeChar.sendMessage("Fusion Item Data has been reloaded.");
					}
